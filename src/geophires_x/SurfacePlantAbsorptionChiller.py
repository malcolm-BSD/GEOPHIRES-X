...
class SurfacePlantAbsorptionChiller(SurfacePlant):
    ...
    def _dispatch_temperature_value(self, parameter, default_value: float, timestep_index: Optional[int]) -> float:
    ...
    @staticmethod
    def _dispatch_bank_one_hour(
        bank,
        cooling_load_kW: float,
        generator_heat_available_kW: float,
        t_gen_c: float,
        t_evap_c: float,
        t_cond_c: float,
    ) -> dict[str, float]:
        """Evaluate the greedy one-hour chiller dispatch path without array setup."""
        unit_order = getattr(bank, "_single_hour_unit_order_cache", None)
        if unit_order is None:
            unit_list = []
            for unit, count in bank.units:
                for _ in range(int(count)):
                    unit_list.append(unit)

            unit_order = list(enumerate(unit_list))
            if bank.dispatch_strategy == "min_units":
                unit_order.sort(key=lambda item: item[1].nominal_cooling_kW, reverse=True)
            elif bank.dispatch_strategy == "min_cost":
                def cost_key(unit):
                    try:
                        return float(unit.installed_cost_USD) / float(unit.nominal_cooling_kW)
                    except Exception:
                        return float(unit.nominal_cooling_kW)

                unit_order.sort(key=lambda item: cost_key(item[1]))
            setattr(bank, "_single_hour_unit_order_cache", unit_order)

        cooling_kW = 0.0
        q_gen_kW = 0.0
        remaining_kW = max(float(cooling_load_kW), 0.0)
        for _, unit in unit_order:
            if remaining_kW <= 0.0:
                break

            max_unit_capacity_kW = float(unit.nominal_cooling_kW)
            if max_unit_capacity_kW <= 0.0:
                continue

            desired_plr = min(1.0, remaining_kW / max_unit_capacity_kW)
            if 0.0 < desired_plr < unit.min_PLR:
                if unit.min_PLR * max_unit_capacity_kW <= remaining_kW:
                    plr = unit.min_PLR
                else:
                    continue
            else:
            ...
            perf = unit.performance_at_plr(plr, t_gen_c, t_evap_c, t_cond_c)
            cooling_kW += float(perf["cooling_kW"])
            q_gen_kW += float(perf["fuel_input_kW"])
            remaining_kW = max(0.0, remaining_kW - float(perf["cooling_kW"]))

        if q_gen_kW > generator_heat_available_kW:
            scale = generator_heat_available_kW / q_gen_kW if q_gen_kW > 0.0 else 0.0
            cooling_kW *= scale
            q_gen_kW = generator_heat_available_kW

        pump_power_kW = 0.0
        if cooling_kW > 0.0 and unit_order:
            cp_j_per_kg_k = 4186.0
            chilled_delta_t_k = 5.0
            chilled_mdot_kg_per_sec = (cooling_kW * 1000.0) / (cp_j_per_kg_k * chilled_delta_t_k)
            water_density_kg_per_m3 = 997.0
            gravity_m_per_s2 = 9.81
            pump_head_m = unit_order[0][1].pump_head_m or 30.0
            volume_flow_m3_per_sec = chilled_mdot_kg_per_sec / water_density_kg_per_m3
            hydraulic_power_w = water_density_kg_per_m3 * gravity_m_per_s2 * pump_head_m * volume_flow_m3_per_sec
            pump_power_kW = hydraulic_power_w / 0.7 / 1000.0

        return {
            "cooling_produced_kW": cooling_kW,
            "q_gen_kW": q_gen_kW,
            "unmet_cooling_kW": max(0.0, float(cooling_load_kW) - cooling_kW),
            "cop": cooling_kW / q_gen_kW if q_gen_kW > 0.0 else 0.0,
    ...
    def advanced_dispatch_output(
        ...
        results = self._dispatch_bank_one_hour(
            bank,
            float(cooling_demand_mw) * 1000.0,
            float(generator_heat_available_mw) * 1000.0,
            float(temps["t_gen"][0]),
            float(temps["t_evap"][0]),
            float(temps["t_cond"][0]),
        )
        return {
            "cooling_produced_mw": float(results["cooling_produced_kW"]) / 1000.0,
            "q_gen_mw": float(results["q_gen_kW"]) / 1000.0,
            "unmet_cooling_mw": float(results["unmet_cooling_kW"]) / 1000.0,
            "cop": float(results["cop"]),
            "pump_power_mw": float(results["pump_power_kW"]) / 1000.0,
        }
